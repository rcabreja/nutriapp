  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold text-[#3c584b]">Pacientes</h2>
        <button
          onClick={() => setShowCreateModal(true)}
          className="w-full sm:w-auto bg-[#cbd9ce] hover:opacity-90 text-[#3c584b] px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors"
        >
      ...
      <div className="bg-[#fdf7e7] border border-[#cbd9ce] rounded-xl overflow-hidden">
        <div className="p-4 border-b border-[#cbd9ce]">
          <div className="relative w-full sm:max-w-md">
            ...
            <input
              ...
              className="w-full bg-[#fdf7e7] border border-[#cbd9ce] rounded-lg py-2.5 pl-10 pr-4 text-[#3c584b] focus:outline-none focus:border-[#cbd9ce] transition-colors"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left whitespace-nowrap">
            <thead className="bg-[#fdf7e7] text-[#3c584b] text-xs uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800">
              {filteredPatients.map((patient) => (
                <tr key={patient.id} className="hover:bg-[#fdf7e7] transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#cbd9ce] text-[#3c584b] flex items-center justify-center text-sm font-bold overflow-hidden shrink-0">
                        {patient.avatarUrl ? <img src={patient.avatarUrl} alt="" className="w-full h-full object-cover" /> : patient.name.charAt(0)}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-medium text-[#3c584b]">{patient.name}</span>
                        <span className="text-xs text-[#3c584b] md:hidden">{patient.email}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-[#3c584b] hidden md:table-cell">{patient.email}</td>
                  <td className="px-6 py-4 text-[#3c584b] hidden sm:table-cell">{patient.phone}</td>
                  <td className="px-6 py-4 text-[#3c584b] hidden sm:table-cell">
                    {patient.dob ? new Date().getFullYear() - new Date(patient.dob).getFullYear() : '-'} años
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link to={`/patients/${patient.id}`} className="p-2 text-[#3c584b] hover:bg-[#cbd9ce] rounded-lg transition-colors">
                        <Eye size={18} />
                      </Link>
                      <button
                        onClick={() => setPatientToDelete(patient.id)}
                        className="p-2 text-[#3c584b] hover:bg-[#cbd9ce] rounded-lg transition-colors"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
              ...
              {filteredPatients.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-[#3c584b]">
      ...
      {patientToDelete && (
        <div className="fixed inset-0 bg-[#fdf7e7] backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <div className="bg-[#fdf7e7] border border-[#cbd9ce] rounded-xl p-8 max-w-sm w-full shadow-2xl text-center animate-in fade-in zoom-in duration-200">
            ...
            <h3 className="text-xl font-bold text-[#3c584b] mb-2">¿Eliminar Paciente?</h3>
